...
export default function EditMemberForm() {
    ...
    const fetchMember = async () => {
      try {
        ...
        if (snap.exists()) {
          ...
          // ลองโหลดทั้ง .jpg และ .png
          try {
            const jpgUrl = await getDownloadURL(ref(storage, `party/member/${partyName}/${memberId}.jpg`));
            setPreviewUrl(jpgUrl);
          } catch {
            try {
              const pngUrl = await getDownloadURL(ref(storage, `party/member/${partyName}/${memberId}.png`));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (imageFile) {
      const imageRef = ref(storage, `party/member/${partyName}/${memberId}.jpg`);
