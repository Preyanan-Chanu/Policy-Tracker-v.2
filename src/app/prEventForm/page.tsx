...
// ✅ โหลด campaign เมื่อเลือกนโยบาย
useEffect(() => {
  setSelectedCampaign(""); 
  setCampaigns([]);
  if (policyName) {
...
// ✅ โหลดข้อมูลกิจกรรมเดิม (สำหรับแก้ไข)
useEffect(() => {
  if (!eventId) return;
...
// ✅ handle submit (เพิ่มหรือแก้ไขกิจกรรม)
...
useEffect(() => {
  if (eventTime.includes("-")) {
    ...
    setEndTime(""); // หรือ default เป็น 1 ชม. หลัง start ก็ได้
  }
}, [eventTime]);
...
>
  ...
  onClick={(e) => {
    ...
    // 🔍 ดึงที่อยู่จากพิกัดด้วย Geocoder
